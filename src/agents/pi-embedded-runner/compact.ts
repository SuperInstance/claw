...
import { applySkillEnvOverrides, applySkillEnvOverridesFromSnapshot, resolveSkillsPromptForRun, type SkillSnapshot, } from "../skills.js";
 ...
 */
export async function compactEmbeddedPiSessionDirect(params: CompactEmbeddedPiSessionParams): Promise<EmbeddedPiCompactResult> {
    ...
    try {
        ...
        try {
            ...
            if (model.api === "ollama") {
                const providerBaseUrl = typeof params.config?.models?.providers?.[model.provider]?.baseUrl === "string"
                    ? params.config.models.providers[model.provider]?.baseUrl
                    : undefined;
                ensureCustomApiRegistered(model.api, createConfiguredOllamaStreamFn({
                    model,
                    providerBaseUrl,
                }));
            }
            try {
                ...
                const result = await compactWithSafetyTimeout(() => session.compact(params.customInstructions), compactionTimeoutMs, {
 ...
 */
export async function compactEmbeddedPiSession(params: CompactEmbeddedPiSessionParams): Promise<EmbeddedPiCompactResult> {
