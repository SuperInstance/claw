...
import { makeZeroUsageSnapshot, normalizeUsage, type AssistantUsageSnapshot, type UsageLike, } from "../usage.js";
...
function buildInterSessionPrefix(message: AgentMessage): string {
    const provenance = normalizeInputProvenance((message as {
        provenance?: unknown;
    }).provenance);
    if (!provenance) {
...
function annotateInterSessionUserMessages(messages: AgentMessage[]): AgentMessage[] {
    ...
    for (const msg of messages) {
        if (!hasInterSessionUserProvenance(msg as {
            role?: unknown;
            provenance?: unknown;
        })) {
            out.push(msg);
        ...
        const user = msg as Extract<AgentMessage, {
            role: "user";
        }>;
...
function stripStaleAssistantUsageBeforeLatestCompaction(messages: AgentMessage[]): AgentMessage[] {
    ...
    for (let i = 0; i < messages.length; i += 1) {
        ...
        latestCompactionTimestamp = parseMessageTimestamp((entry as {
            timestamp?: unknown;
        }).timestamp ?? null);
    }
    ...
    for (let i = 0; i < out.length; i += 1) {
        const candidate = out[i] as (AgentMessage & {
            usage?: unknown;
            timestamp?: unknown;
        }) | undefined;
...
function ensureAssistantUsageSnapshots(messages: AgentMessage[]): AgentMessage[] {
    ...
    const out = [...messages];
    for (let i = 0; i < out.length; i += 1) {
        const message = out[i] as (AgentMessage & {
            role?: unknown;
            usage?: unknown;
        }) | undefined;
        ...
        if (message.usage &&
            ...
            ((normalizedCost &&
                ...
                (usageCost as {
                    input?: unknown;
                }).input === normalizedCost.input &&
                (usageCost as {
                    output?: unknown;
                }).output === normalizedCost.output &&
                (usageCost as {
                    cacheRead?: unknown;
                }).cacheRead === normalizedCost.cacheRead &&
                (usageCost as {
                    cacheWrite?: unknown;
                }).cacheWrite === normalizedCost.cacheWrite &&
                (usageCost as {
                    total?: unknown;
                }).total === normalizedCost.total) ||
...
function hasGoogleTurnOrderingMarker(sessionManager: SessionManager): boolean {
    try {
        return sessionManager
            .getEntries()
            .some((entry) => (entry as CustomEntryLike)?.type === "custom" &&
