...
import { isProfileInCooldown, type AuthProfileFailureReason, markAuthProfileFailure, markAuthProfileGood, markAuthProfileUsed, resolveProfilesUnavailableReason, } from "../auth-profiles.js";
...
import { applyLocalNoAuthHeaderOverride, ensureAuthProfileStore, getApiKeyForModel, resolveAuthProfileOrder, type ResolvedProviderAuth, } from "../model-auth.js";
...
const hasUsageValues = (usage: ReturnType<typeof normalizeUsage>): usage is NonNullable<ReturnType<typeof normalizeUsage>> => !!usage &&
...
const mergeUsageIntoAccumulator = (target: UsageAccumulator, usage: ReturnType<typeof normalizeUsage>) => {
...
export async function runEmbeddedPiAgent(params: RunEmbeddedPiAgentParams): Promise<EmbeddedPiRunResult> {
    ...
    return enqueueSession(() => enqueueGlobal(async () => {
        ...
        let effectiveModel = ctxInfo.tokens < (runtimeModel.contextWindow ?? Infinity)
        ...
        const refreshRuntimeAuth = async (reason: string): Promise<void> => {
            ...
            runtimeAuthState.refreshInFlight = (async () => {
                ...
                if (preparedAuth.expiresAt) {
                    ...
                    log.debug(`Runtime auth refreshed for ${runtimeModel.provider}; expires in ${Math.max(0, Math.floor(remaining / 1000))}s.`);
                }
            })()
                .catch((err) => {
                log.warn(`Runtime auth refresh failed for ${runtimeModel.provider}: ${describeUnknownError(err)}`);
                throw err;
            })
                .finally(() => {
                if (runtimeAuthState) {
        ...
        const scheduleRuntimeAuthRefresh = (): void => {
            ...
            if (!hasRefreshableRuntimeAuth()) {
                log.warn(`Skipping runtime auth refresh scheduling for ${runtimeModel.provider}; source credential missing.`);
                return;
            }
        ...
        const resolveAuthProfileFailoverReason = (params: {
            ...
            if (params.allInCooldown) {
                const profileIds = (params.profileIds ?? profileCandidates).filter((id): id is string => typeof id === "string" && id.length > 0);
        ...
        try {
            const autoProfileCandidates = profileCandidates.filter((candidate): candidate is string => typeof candidate === "string" && candidate.length > 0 && candidate !== lockedProfileId);
            ...
            const unavailableReason = allAutoProfilesInCooldown
                ...
                : null;
            const allowTransientCooldownProbe = params.allowTransientCooldownProbe === true &&
            ...
            while (profileIndex < profileCandidates.length) {
                ...
                const inCooldown = candidate && candidate !== lockedProfileId && isProfileInCooldown(authStore, candidate);
                if (inCooldown) {
                    if (allowTransientCooldownProbe && !didTransientCooldownProbe) {
                        didTransientCooldownProbe = true;
                        log.warn(`probing cooldowned auth profile for ${provider}/${modelId} due to ${unavailableReason ?? "transient"} unavailability`);
                    }
                    else {
                        profileIndex += 1;
                        continue;
        ...
        const maybeRefreshRuntimeAuthForAuthError = async (errorText: string, retried: boolean): Promise<boolean> => {
        ...
        const resolveAuthProfileFailureReason = (failoverReason: FailoverReason | null): AuthProfileFailureReason | null => {
                ...
                if (contextOverflowError) {
                    ...
                    if (!isCompactionFailure &&
                        ...
                        if (overflowHookRunner?.hasHooks("before_compaction")) {
                            try {
                                await overflowHookRunner.runBeforeCompaction({ messageCount: -1, sessionFile: params.sessionFile }, hookCtx);
                            }
                            catch (hookErr) {
                                log.warn(`before_compaction hook failed during overflow recovery: ${String(hookErr)}`);
                            }
                        }
                        try {
                        ...
                        if (compactResult.ok &&
                            ...
                            try {
                                ...
                                log.warn(`after_compaction hook failed during overflow recovery: ${String(hookErr)}`);
                            }
                        }
