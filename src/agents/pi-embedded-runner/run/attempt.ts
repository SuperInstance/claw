...
import { analyzeBootstrapBudget, buildBootstrapPromptWarning, buildBootstrapTruncationReportMeta, buildBootstrapInjectionStats, } from "../../bootstrap-budget.js";
...
import { clearActiveEmbeddedRun, type EmbeddedPiQueueHandle, setActiveEmbeddedRun, updateActiveEmbeddedRunSnapshot, } from "../runs.js";
...
async function persistSessionsYieldContextMessage(activeSession: {
...
function stripSessionsYieldArtifacts(activeSession: {
    ...
    while (strippedMessages.length > 0) {
        const last = strippedMessages.at(-1) as AgentMessage | {
            role?: string;
            customType?: string;
            stopReason?: string;
        };
...
function resolveCaseInsensitiveAllowedToolName(rawName: string, allowedToolNames?: Set<string>): string | null {
...
function resolveExactAllowedToolName(rawName: string, allowedToolNames?: Set<string>): string | null {
    if (!allowedToolNames || allowedToolNames.size === 0) {
        return null;
    }
...
function resolveStructuredAllowedToolName(rawName: string, allowedToolNames?: Set<string>): string | null {
    if (!allowedToolNames || allowedToolNames.size === 0) {
        return null;
    }
...
function inferToolNameFromToolCallId(rawId: string | undefined, allowedToolNames?: Set<string>): string | null {
...
function normalizeToolCallNameForDispatch(rawName: string, allowedToolNames?: Set<string>, rawToolCallId?: string): string {
    const trimmed = rawName.trim();
    if (!trimmed) {
...
function trimWhitespaceFromToolCallNamesInMessage(message: unknown, allowedToolNames?: Set<string>): void {
    if (!message || typeof message !== "object") {
        return;
    }
    const content = (message as {
...
function wrapStreamTrimToolCallNames(stream: ReturnType<typeof streamSimple>, allowedToolNames?: Set<string>): ReturnType<typeof streamSimple> {
    ...
    (stream as {
        [Symbol.asyncIterator]: typeof originalAsyncIterator;
...
export function wrapStreamFnTrimToolCallNames(baseFn: StreamFn, allowedToolNames?: Set<string>): StreamFn {
...
const MAX_TOOLCALL_REPAIR_BUFFER_CHARS = 64000;
...
function repairToolCallArgumentsInMessage(message: unknown, contentIndex: number, repairedArgs: Record<string, unknown>): void {
    if (!message || typeof message !== "object") {
        return;
    }
    const content = (message as {
...
function repairMalformedToolCallArgumentsInMessage(message: unknown, repairedArgsByIndex: Map<number, Record<string, unknown>>): void {
    if (!message || typeof message !== "object") {
        return;
    }
    const content = (message as {
...
function wrapStreamRepairMalformedToolCallArguments(stream: ReturnType<typeof streamSimple>): ReturnType<typeof streamSimple> {
                            ...
                            if (shouldAttemptMalformedToolCallRepair(nextPartialJson, event.delta)) {
                                ...
                                if (repair) {
                                    ...
                                    if (!loggedRepairIndices.has(event.contentIndex)) {
                                        ...
                                        log.warn(`repairing kimi-coding tool call arguments after ${repair.trailingSuffix.length} trailing chars`);
                                    }
                                }
                                else {
                                    repairedArgsByIndex.delete(event.contentIndex);
...
function wrapStreamDecodeXaiToolCallArguments(stream: ReturnType<typeof streamSimple>): ReturnType<typeof streamSimple> {
    const originalResult = stream.result.bind(stream);
    stream.result = async () => {
        const message = await originalResult();
    ...
    try {
        ...
        const toolsRaw = params.disableTools
            ...
            : createOpenClawCodingTools({
                ...
                spawnWorkspaceDir: sandbox?.enabled && sandbox.workspaceAccess !== "rw" ? resolvedWorkspace : undefined,
                config: params.config,
        ...
        try {
            ...
            }, params.thinkLevel, sessionAgentId);
            if (cacheTrace) {
            ...
            activeSession.agent.streamFn = (model, context, options) => {
                ...
                if (yieldDetected && signal.aborted && signal.reason === "sessions_yield") {
                    return createYieldAbortedResponse(model) as unknown as Awaited<ReturnType<typeof innerStreamFn>>;
                }
            ...
            const getAbortReason = (signal: AbortSignal): unknown => "reason" in signal ? (signal as {
                reason?: unknown;
            }).reason : undefined;
            ...
            try {
                ...
                const transcriptLeafId = (sessionManager.getLeafEntry() as {
                    id?: string;
                ...
                const COMPACTION_RETRY_AGGREGATE_TIMEOUT_MS = 60000;
                try {
