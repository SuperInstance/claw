...
import { ANTHROPIC_CONTEXT_1M_TOKENS, applyConfiguredContextWindows, applyDiscoveredContextWindows, resolveContextTokensForModel, } from "./context.js";
describe("applyDiscoveredContextWindows", () => {
    it("keeps the smallest context window when the same bare model id appears under multiple providers", () => {
        ...
        applyDiscoveredContextWindows({
            cache,
            models: [
                { id: "gemini-3.1-pro-preview", contextWindow: 128000 },
        ...
        expect(cache.get("gemini-3.1-pro-preview")).toBe(128000);
    });
    it("stores provider-qualified entries independently", () => {
        ...
        applyDiscoveredContextWindows({
            cache,
            models: [
                { id: "github-copilot/gemini-3.1-pro-preview", contextWindow: 128000 },
                { id: "google-gemini-cli/gemini-3.1-pro-preview", contextWindow: 1048576 },
            ],
        });
        expect(cache.get("github-copilot/gemini-3.1-pro-preview")).toBe(128000);
        expect(cache.get("google-gemini-cli/gemini-3.1-pro-preview")).toBe(1048576);
    });
});
describe("applyConfiguredContextWindows", () => {
    it("writes bare model id to cache; does not touch raw provider-qualified discovery entries", () => {
        ...
        const cache = new Map<string, number>([["openrouter/anthropic/claude-opus-4-6", 1000000]]);
        ...
        expect(cache.get("openrouter/anthropic/claude-opus-4-6")).toBe(1000000);
    });
    it("does not write synthetic provider-qualified keys; only bare model ids go into cache", () => {
        ...
        const cache = new Map<string, number>();
        cache.set("google-gemini-cli/gemini-3.1-pro-preview", 1048576); // discovery entry
        applyConfiguredContextWindows({
            cache,
            modelsConfig: {
                providers: {
                    "google-gemini-cli": {
                        models: [{ id: "gemini-3.1-pro-preview", contextWindow: 200000 }],
                    },
                },
            },
        });
        // Bare key is written.
        expect(cache.get("gemini-3.1-pro-preview")).toBe(200000);
