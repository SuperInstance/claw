...
function toUsage(raw: Record<string, unknown>): CliUsage | undefined {
    const pick = (key: string) => typeof raw[key] === "number" && raw[key] > 0 ? raw[key] : undefined;
...
function pickSessionId(parsed: Record<string, unknown>, backend: CliBackendConfig): string | undefined {
...
export async function writeCliImages(images: ImageContent[]): Promise<{
    paths: string[];
