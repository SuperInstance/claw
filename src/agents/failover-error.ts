...
function findErrorProperty<T>(err: unknown, reader: (candidate: unknown) => T | undefined, seen: Set<object> = new Set()): T | undefined {
...
export function isTimeoutError(err: unknown): boolean {
    ...
    const cause = "cause" in err ? (err as {
        cause?: unknown;
    }).cause : undefined;
    const reason = "reason" in err ? (err as {
        reason?: unknown;
    }).reason : undefined;
...
export function coerceToFailoverError(err: unknown, context?: {
    provider?: string;
    model?: string;
    profileId?: string;
}): FailoverError | null {
