 ...
 */
function resolveThreadTsFromContext(explicitThreadTs: string | undefined, targetChannel: string, context: SlackActionContext | undefined): string | undefined {
...
export async function handleSlackAction(params: Record<string, unknown>, cfg: OpenClawConfig, context?: SlackActionContext): Promise<AgentToolResult<unknown>> {
    ...
    if (messagingActions.has(action)) {
        ...
        switch (action) {
            ...
            case "readMessages": {
                ...
                const messages = result.messages.map((message) => withNormalizedTimestamp(message as Record<string, unknown>, (message as {
                    ts?: unknown;
                }).ts));
    ...
    if (pinActions.has(action)) {
        ...
        const normalizedPins = pins.map((pin) => {
            const message = pin.message
                ? withNormalizedTimestamp(pin.message as Record<string, unknown>, (pin.message as {
                    ts?: unknown;
                }).ts)
                : pin.message;
