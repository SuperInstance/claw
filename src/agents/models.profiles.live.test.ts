...
import { collectAnthropicApiKeys, isAnthropicBillingError, isAnthropicRateLimitError, } from "./live-auth-keys.js";
...
function resolveTestReasoning(model: Model<Api>): "minimal" | "low" | "medium" | "high" | "xhigh" | undefined {
    if (!model.reasoning) {
        return undefined;
    }
...
describe("resolveLiveSystemPrompt", () => {
    it("adds instructions for openai-codex probes", () => {
        expect(resolveLiveSystemPrompt({
            provider: "openai-codex",
        } as Model<Api>)).toContain("Follow the user's instruction exactly.");
    });
    it("keeps other providers unchanged", () => {
        expect(resolveLiveSystemPrompt({
            provider: "openai",
        } as Model<Api>)).toBeUndefined();
    });
});
async function completeSimpleWithTimeout<TApi extends Api>(model: Model<TApi>, context: Parameters<typeof completeSimple<TApi>>[1], options: Parameters<typeof completeSimple<TApi>>[2], timeoutMs: number) {
...
async function completeOkWithRetry(params: {
    ...
    const runOnce = async (maxTokens: number) => {
        const res = await completeSimpleWithTimeout(params.model, {
...
describeLive("live models (profile keys)", () => {
    it("completes across selected models", async () => {
        ...
        const perModelTimeoutMs = toInt(process.env.OPENCLAW_LIVE_MODEL_TIMEOUT_MS, 30000);
        ...
        const selectedCandidates = capByProviderSpread(candidates, maxModels > 0 ? maxModels : candidates.length, (entry) => entry.model.provider);
        logProgress(`[live-models] selection=${useExplicit ? "explicit" : "modern"}`);
        if (selectedCandidates.length < candidates.length) {
            logProgress(`[live-models] capped to ${selectedCandidates.length}/${candidates.length} via OPENCLAW_LIVE_MAX_MODELS=${maxModels}`);
        }
        ...
        for (const [index, entry] of selectedCandidates.entries()) {
            ...
            for (let attempt = 0; attempt < attemptMax; attempt += 1) {
                ...
                const apiKey = model.provider === "anthropic" && anthropicKeys.length > 0
                ...
                try {
                    ...
                    if (model.provider === "openai" &&
                        ...
                        let first = await completeSimpleWithTimeout(model, { messages: [firstUser], tools: [noopTool] }, {
                            apiKey,
                        ...
                        for (let i = 0; i < 2 && (!toolCall || firstText.length > 0); i += 1) {
                            ...
                            first = await completeSimpleWithTimeout(model, { messages: [firstUser], tools: [noopTool] }, {
                                apiKey,
                                reasoning: resolveTestReasoning(model),
                                maxTokens: 128,
                            }, perModelTimeoutMs);
                        ...
                        const second = await completeSimpleWithTimeout(model, {
                            messages: [
                                firstUser,
                                first,
                                {
                    ...
                    if ((model.provider === "google" || model.provider === "google-gemini-cli") &&
        ...
        if (failures.length > 0) {
            ...
            throw new Error(`live model failures (${failures.length}, showing ${Math.min(failures.length, 20)}):\n${preview}`);
        }
        void skipped;
    }, 15 * 60 * 1000);
});
