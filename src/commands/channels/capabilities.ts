...
function normalizeTimeout(raw: unknown, fallback = 10000) {
...
function formatProbeLines(channelId: string, probe: unknown): string[] {
    ...
    if (channelId === "telegram") {
        ...
        const flags: string[] = [];
        const canJoinGroups = (bot as {
            canJoinGroups?: boolean | null;
        })?.canJoinGroups;
        const canReadAll = (bot as {
            canReadAllGroupMessages?: boolean | null;
        })
            ?.canReadAllGroupMessages;
        const inlineQueries = (bot as {
        ...
        const webhook = probeObj.webhook as {
            url?: string | null;
        } | undefined;
...
async function resolveChannelReports(params: {
    ...
    for (const accountId of accountIds) {
        ...
        if (plugin.id === "slack" && configured && enabled) {
            const botToken = (resolvedAccount as {
                botToken?: string;
            }).botToken?.trim();
            const userToken = (resolvedAccount as {
                userToken?: string;
            }).userToken?.trim();
        ...
        reports.push({
            channel: plugin.id,
            accountId,
            accountName: typeof (resolvedAccount as {
                name?: string;
            }).name === "string"
                ? (resolvedAccount as {
                    name?: string;
...
export async function channelsCapabilitiesCommand(opts: ChannelsCapabilitiesOptions, runtime: RuntimeEnv = defaultRuntime) {
    const cfg = await requireValidConfig(runtime);
    if (!cfg) {
        return;
    }
    const timeoutMs = normalizeTimeout(opts.timeout, 10000);
