...
vi.mock("../../config/sessions/transcript.js", async () => {
    const actual = await vi.importActual<typeof import("../../config/sessions/transcript.js")>("../../config/sessions/transcript.js");
    return {
        ...actual,
        appendAssistantMessageToSessionTranscript: mocks.appendAssistantMessageToSessionTranscript,
    };
...
function expectSuccessfulWhatsAppInternalHookPayload(expected: Partial<{
    content: string;
    messageId: string;
    isGroup: boolean;
    groupId: string;
...
describe("deliverOutboundPayloads", () => {
    ...
    it("clamps telegram text chunk size to protocol max even with higher config", async () => {
        const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
        const cfg: OpenClawConfig = {
            channels: { telegram: { botToken: "tok-1", textChunkLimit: 10000 } },
        };
    ...
    it("passes explicit accountId to sendTelegram", async () => {
        ...
        expect(sendTelegram).toHaveBeenCalledWith("123", "hi", expect.objectContaining({ accountId: "default", verbose: false, textMode: "html" }));
    });
    it("formats BTW replies prominently for telegram delivery", async () => {
        ...
        expect(sendTelegram).toHaveBeenCalledWith("123", "BTW\nQuestion: what is 17 * 19?\n\n323", expect.objectContaining({ verbose: false, textMode: "html" }));
    });
    ...
    it("does not inject telegram approval buttons from plain approval text", async () => {
        ...
        const sendOpts = sendTelegram.mock.calls[0]?.[2] as {
            buttons?: unknown;
        } | undefined;
    ...
    it("scopes media local roots to the active agent workspace when agentId is provided", async () => {
        ...
        expect(sendTelegram).toHaveBeenCalledWith("123", "hi", expect.objectContaining({
    ...
    it("includes OpenClaw tmp root in signal mediaLocalRoots", async () => {
        ...
        expect(sendSignal).toHaveBeenCalledWith("+1555", "hi", expect.objectContaining({
            mediaLocalRoots: expect.arrayContaining([resolvePreferredOpenClawTmpDir()]),
        }));
    });
    it("includes OpenClaw tmp root in whatsapp mediaLocalRoots", async () => {
        ...
        expect(sendWhatsApp).toHaveBeenCalledWith("+1555", "hi", expect.objectContaining({
            mediaLocalRoots: expect.arrayContaining([resolvePreferredOpenClawTmpDir()]),
        }));
    });
    it("includes OpenClaw tmp root in imessage mediaLocalRoots", async () => {
        ...
        expect(sendIMessage).toHaveBeenCalledWith("imessage:+15551234567", "hi", expect.objectContaining({
            mediaLocalRoots: expect.arrayContaining([resolvePreferredOpenClawTmpDir()]),
        }));
    });
    ...
    it("chunks Signal markdown using the format-first chunker", async () => {
        ...
        expectedChunks.forEach((chunk, index) => {
            expect(sendSignal).toHaveBeenNthCalledWith(index + 1, "+1555", chunk.text, expect.objectContaining({
                accountId: undefined,
                textMode: "plain",
    ...
    it("strips leading blank lines for WhatsApp text payloads", async () => {
        ...
        expect(sendWhatsApp).toHaveBeenNthCalledWith(1, "+1555", "Hello from WhatsApp", expect.objectContaining({ verbose: false }));
    });
    ...
    it("keeps WhatsApp media payloads but clears whitespace-only captions", async () => {
        ...
        expect(sendWhatsApp).toHaveBeenCalledTimes(1);
        expect(sendWhatsApp).toHaveBeenNthCalledWith(1, "+1555", "", expect.objectContaining({
            mediaUrl: "https://example.com/photo.png",
            verbose: false,
        }));
    });
    ...
    it("uses iMessage media maxBytes from agent fallback", async () => {
        ...
        expect(sendIMessage).toHaveBeenCalledWith("chat_id:42", "hello", expect.objectContaining({ maxBytes: 3 * 1024 * 1024 }));
    });
    ...
    it("formats BTW replies prominently for whatsapp delivery", async () => {
        ...
        expect(sendWhatsApp).toHaveBeenCalledWith("+1555", "BTW\nQuestion: what is 17 * 19?\n\n323", expect.any(Object));
    });
    ...
    it("emits internal message:sent hook with success=true for chunked payload delivery", async () => {
        ...
        expect(internalHookMocks.createInternalHookEvent).toHaveBeenCalledWith("message", "sent", "agent:main:main", expectSuccessfulWhatsAppInternalHookPayload({
            content: "abcd",
            messageId: "w2",
            isGroup: true,
            groupId: "whatsapp:group:123",
    ...
    it("emits internal message:sent hook when sessionKey is provided without mirror", async () => {
        ...
        expect(internalHookMocks.createInternalHookEvent).toHaveBeenCalledTimes(1);
        expect(internalHookMocks.createInternalHookEvent).toHaveBeenCalledWith("message", "sent", "agent:main:main", expectSuccessfulWhatsAppInternalHookPayload({ content: "hello", messageId: "w1" }));
        expect(internalHookMocks.triggerInternalHook).toHaveBeenCalledTimes(1);
    });
    it("warns when session.agentId is set without a session key", async () => {
        ...
        expect(logMocks.warn).toHaveBeenCalledWith("deliverOutboundPayloads: session.agentId present without session key; internal message:sent hook will be skipped", expect.objectContaining({ channel: "whatsapp", to: "+1555", agentId: "agent-main" }));
    });
    ...
    it("acks the queue entry when delivery is aborted", async () => {
        ...
        const cfg: OpenClawConfig = {};
        await expect(deliverOutboundPayloads({
            cfg,
            channel: "whatsapp",
            to: "+1555",
            payloads: [{ text: "a" }],
    ...
    it("passes normalized payload to onError", async () => {
        ...
        expect(onError).toHaveBeenCalledTimes(1);
        expect(onError).toHaveBeenCalledWith(expect.any(Error), expect.objectContaining({ text: "hi", mediaUrls: ["https://x.test/a.jpg"] }));
    });
    it("mirrors delivered output when mirror options are provided", async () => {
        ...
        expect(mocks.appendAssistantMessageToSessionTranscript).toHaveBeenCalledWith(expect.objectContaining({
            text: "report.pdf",
            idempotencyKey: "idem-deliver-1",
        }));
    });
    it("emits message_sent success for text-only deliveries", async () => {
        ...
        expect(hookMocks.runner.runMessageSent).toHaveBeenCalledWith(expect.objectContaining({ to: "+1555", content: "hello", success: true }), expect.objectContaining({ channelId: "whatsapp" }));
    });
    it("emits message_sent success for sendPayload deliveries", async () => {
        ...
        expect(hookMocks.runner.runMessageSent).toHaveBeenCalledWith(expect.objectContaining({ to: "!room:1", content: "payload text", success: true }), expect.objectContaining({ channelId: "matrix" }));
    });
    ...
    it("falls back to sendText when plugin outbound omits sendMedia", async () => {
        ...
        expect(logMocks.warn).toHaveBeenCalledWith("Plugin outbound adapter does not implement sendMedia; media URLs will be dropped and text fallback will be used", expect.objectContaining({
            channel: "matrix",
            mediaCount: 1,
        }));
    ...
    it("fails media-only payloads when plugin outbound omits sendMedia", async () => {
        ...
        })).rejects.toThrow("Plugin outbound adapter does not implement sendMedia and no text fallback is available for media payload");
        expect(sendText).not.toHaveBeenCalled();
        expect(logMocks.warn).toHaveBeenCalledWith("Plugin outbound adapter does not implement sendMedia; media URLs will be dropped and text fallback will be used", expect.objectContaining({
            channel: "matrix",
            mediaCount: 1,
        }));
        expect(hookMocks.runner.runMessageSent).toHaveBeenCalledWith(expect.objectContaining({
            to: "!room:1",
            content: "",
            success: false,
            error: "Plugin outbound adapter does not implement sendMedia and no text fallback is available for media payload",
