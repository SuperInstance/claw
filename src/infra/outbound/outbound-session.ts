...
function resolveDiscordOutboundTargetKindHint(params: ResolveOutboundSessionRouteParams): DiscordTargetKind | undefined {
    const resolvedKind = params.resolvedTarget?.kind;
    if (resolvedKind === "user") {
        return "user";
    }
...
function resolveZaloLikeSession(params: ResolveOutboundSessionRouteParams, channel: "zalo" | "zalouser", aliasPrefix: RegExp): OutboundSessionRoute | null {
...
type OutboundSessionResolver = (params: ResolveOutboundSessionRouteParams) => OutboundSessionRoute | null | Promise<OutboundSessionRoute | null>;
...
export async function resolveOutboundSessionRoute(params: ResolveOutboundSessionRouteParams): Promise<OutboundSessionRoute | null> {
    const target = params.target.trim();
    if (!target) {
        return null;
    }
