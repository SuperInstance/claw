...
vi.mock("../../../extensions/whatsapp/src/media.js", async () => {
    const actual = await vi.importActual<typeof import("../../../extensions/whatsapp/src/media.js")>("../../../extensions/whatsapp/src/media.js");
    return {
        ...actual,
...
async function expectSandboxMediaRewrite(params: {
    ...
    expect(result.sendResult?.mediaUrl).toBe(path.join(params.sandboxDir, params.expectedRelativePath));
}
...
describe("runMessageAction media behavior", () => {
    ...
    describe("sendAttachment hydration", () => {
        ...
        it("hydrates buffer and filename from media for sendAttachment", async () => {
            ...
            expect((result.payload as {
                buffer?: string;
            }).buffer).toBe(Buffer.from("hello").toString("base64"));
            ...
            } | undefined)?.sandboxValidated).not.toBe(true);
        });
    ...
    describe("sandboxed media validation", () => {
        ...
        it.each(["/etc/passwd", "file:///etc/passwd"])("rejects out-of-sandbox media reference: %s", async (media) => {
            await withSandbox(async (sandboxDir) => {
                await expect(runDrySend({
                    cfg: slackConfig,
                    actionParams: {
                        channel: "slack",
                        target: "#C12345678",
