...
export async function routeReply(params: RouteReplyParams): Promise<RouteReplyResult> {
    ...
    const normalized = normalizeReplyPayload(payload, {
        responsePrefix,
        enableSlackInteractiveReplies: channel === "slack" ? isSlackInteractiveRepliesEnabled({ cfg, accountId }) : false,
    });
    if (!normalized) {
        return { ok: true };
    }
    ...
    if (channel === "slack" &&
        ...
        try {
            hasSlackBlocks = Boolean(parseSlackBlocksInput((externalPayload.channelData.slack as {
                blocks?: unknown;
            }).blocks)
                ?.length);
        }
 ...
 */
export function isRoutableChannel(channel: OriginatingChannelType | undefined): channel is Exclude<OriginatingChannelType, typeof INTERNAL_MESSAGE_CHANNEL> {
