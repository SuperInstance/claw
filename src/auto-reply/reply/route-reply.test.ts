...
vi.mock("../../infra/outbound/deliver-runtime.js", async () => {
    const actual = await vi.importActual<typeof import("../../infra/outbound/deliver-runtime.js")>("../../infra/outbound/deliver-runtime.js");
    return {
        ...actual,
        deliverOutboundPayloads: mocks.deliverOutboundPayloads,
    };
});
const actualDeliver = await vi.importActual<typeof import("../../infra/outbound/deliver-runtime.js")>("../../infra/outbound/deliver-runtime.js");
...
async function expectSlackNoSend(payload: Parameters<typeof routeReply>[0]["payload"], overrides: Partial<Parameters<typeof routeReply>[0]> = {}) {
...
describe("routeReply", () => {
    ...
    it("does not drop payloads that merely start with the silent token", async () => {
        ...
        expect(res.ok).toBe(true);
        expect(mocks.sendMessageSlack).toHaveBeenCalledWith("channel:C123", `${SILENT_REPLY_TOKEN} -- (why am I here?)`, expect.any(Object));
    });
    it("applies responsePrefix when routing", async () => {
        ...
        expect(mocks.sendMessageSlack).toHaveBeenCalledWith("channel:C123", "[openclaw] hi", expect.any(Object));
    });
    it("routes directive-only Slack replies when interactive replies are enabled", async () => {
        ...
        expect(mocks.sendMessageSlack).toHaveBeenCalledWith("channel:C123", "", expect.objectContaining({
            blocks: [
    ...
    it("uses threadId for Slack when replyToId is missing", async () => {
        ...
        expect(mocks.sendMessageSlack).toHaveBeenCalledWith("channel:C123", "hi", expect.objectContaining({ threadTs: "456.789" }));
    });
    it("passes thread id to Telegram sends", async () => {
        ...
        expect(mocks.sendMessageTelegram).toHaveBeenCalledWith("telegram:123", "hi", expect.objectContaining({ messageThreadId: 42 }));
    });
    it("formats BTW replies prominently on routed sends", async () => {
        ...
        expect(mocks.sendMessageSlack).toHaveBeenCalledWith("channel:C123", "BTW\nQuestion: what is 17 * 19?\n\n323", expect.any(Object));
    });
    it("formats BTW replies prominently on routed discord sends", async () => {
        ...
        expect(mocks.sendMessageDiscord).toHaveBeenCalledWith("channel:123456", "BTW\nQuestion: what is 17 * 19?\n\n323", expect.any(Object));
    });
    it("passes replyToId to Telegram sends", async () => {
        ...
        expect(mocks.sendMessageTelegram).toHaveBeenCalledWith("telegram:123", "hi", expect.objectContaining({ replyToMessageId: 123 }));
    });
    it("uses replyToId as threadTs for Slack", async () => {
        ...
        expect(mocks.sendMessageSlack).toHaveBeenCalledWith("channel:C123", "hi", expect.objectContaining({ threadTs: "1710000000.0001" }));
    });
    it("uses threadId as threadTs for Slack when replyToId is missing", async () => {
        ...
        expect(mocks.sendMessageSlack).toHaveBeenCalledWith("channel:C123", "hi", expect.objectContaining({ threadTs: "1710000000.9999" }));
    });
    it("uses threadId as replyToId for Mattermost when replyToId is missing", async () => {
        ...
        expect(mocks.deliverOutboundPayloads).toHaveBeenCalledWith(expect.objectContaining({
            channel: "mattermost",
            to: "channel:CHAN1",
            replyToId: "post-root",
            threadId: "post-root",
    ...
    it("sends multiple mediaUrls (caption only on first)", async () => {
        ...
        expect(mocks.sendMessageSlack).toHaveBeenNthCalledWith(1, "channel:C123", "caption", expect.objectContaining({ mediaUrl: "a" }));
        expect(mocks.sendMessageSlack).toHaveBeenNthCalledWith(2, "channel:C123", "", expect.objectContaining({ mediaUrl: "b" }));
    });
    it("routes WhatsApp via outbound sender (accountId honored)", async () => {
        ...
        expect(mocks.sendMessageWhatsApp).toHaveBeenCalledWith("+15551234567", "hi", expect.objectContaining({ accountId: "acc-1", verbose: false }));
    });
    it("routes MS Teams via proactive sender", async () => {
        ...
        expect(mocks.sendMessageMSTeams).toHaveBeenCalledWith(expect.objectContaining({
            cfg,
            to: "conversation:19:abc@thread.tacv2",
            text: "hi",
        }));
