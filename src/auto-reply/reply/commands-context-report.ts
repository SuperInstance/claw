...
function formatListTop(entries: Array<{
    name: string;
    value: number;
}>, cap: number): {
    lines: string[];
...
async function resolveContextReport(params: HandleCommandsParams): Promise<SessionSystemPromptReport> {
...
export async function buildContextReply(params: HandleCommandsParams): Promise<ReplyPayload> {
    ...
    const formatNameList = (names: string[], cap: number) => names.length <= cap
        ? names.join(", ")
