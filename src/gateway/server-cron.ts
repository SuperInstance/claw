...
const CRON_WEBHOOK_TIMEOUT_MS = 10000;
...
export function buildGatewayCronService(params: {
    ...
    const resolveSessionStorePath = (agentId?: string) => resolveStorePath(params.cfg.session?.store, {
    ...
    const cron = new CronService({
        ...
        runHeartbeatOnce: async (opts) => {
            ...
            const agentEntry = Array.isArray(runtimeConfig.agents?.list) &&
                runtimeConfig.agents.list.find((entry) => entry && typeof entry.id === "string" && normalizeAgentId(entry.id) === agentId);
                    ...
                    if (failureDest) {
                        ...
                        if (!isBestEffort) {
                            ...
                            if (failureDest.mode === "webhook" && failureDest.to) {
                                ...
                                const { agentId, cfg: runtimeConfig } = resolveCronAgent(job.agentId);
                                void sendFailureNotificationAnnounce(params.deps, runtimeConfig, agentId, job.id, {
