...
import { AUTH_RATE_LIMIT_SCOPE_HOOK_AUTH, createAuthRateLimiter, normalizeRateLimitClientIp, type AuthRateLimiter, } from "./auth-rate-limit.js";
import { authorizeHttpGatewayConnect, isLocalDirectRequest, type GatewayAuthResult, type ResolvedGatewayAuth, } from "./auth.js";
...
import { extractHookToken, getHookAgentPolicyError, getHookChannelError, type HookAgentDispatchPayload, type HooksConfigResolved, isHookAgentAllowed, normalizeAgentPayload, normalizeHookHeaders, resolveHookIdempotencyKey, normalizeWakePayload, readJsonBody, normalizeHookDispatchSessionKey, resolveHookSessionKey, resolveHookTargetAgentId, resolveHookChannel, resolveHookDeliver, } from "./hooks.js";
...
import { authorizeCanvasRequest, enforcePluginRouteGatewayAuth, isCanvasPath, } from "./server/http-auth.js";
import { isProtectedPluginRoutePathFromContext, resolvePluginRoutePathContext, type PluginHttpRequestHandler, type PluginRoutePathContext, } from "./server/plugins-http.js";
...
function resolveMattermostSlashCallbackPaths(configSnapshot: ReturnType<typeof loadConfig>): Set<string> {
    ...
    const isMattermostCommandCallbackPath = (path: string): boolean => path === MATTERMOST_SLASH_CALLBACK_PATH || path.startsWith("/api/channels/mattermost/");
...
function shouldEnforceDefaultPluginGatewayAuth(pathContext: PluginRoutePathContext): boolean {
    return (pathContext.malformedEncoding ||
...
async function handleGatewayProbeRequest(req: IncomingMessage, res: ServerResponse, requestPath: string, resolvedAuth: ResolvedGatewayAuth, trustedProxies: string[], allowRealIpFallback: boolean, getReadiness?: ReadinessChecker): Promise<boolean> {
...
function writeUpgradeAuthFailure(socket: {
...
export function createHooksRequestHandler(opts: {
    ...
    const resolveHookClientKey = (req: IncomingMessage): string => {
        ...
        const clientIp = resolveRequestClientIp(req, clientIpConfig?.trustedProxies, clientIpConfig?.allowRealIpFallback === true) ?? req.socket?.remoteAddress;
