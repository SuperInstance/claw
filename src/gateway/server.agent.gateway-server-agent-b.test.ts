...
import { agentCommand, connectOk, connectWebchatClient, installGatewayTestHooks, onceMessage, rpcReq, startConnectedServerWithClient, startServerWithClient, testState, trackConnectChallengeNonce, withGatewayServer, writeSessionStore, } from "./test-helpers.js";
...
function sendAgentWsRequest(socket: WebSocket, params: {
    reqId: string;
    message: string;
...
async function sendAgentWsRequestAndWaitFinal(socket: WebSocket, params: {
    reqId: string;
    message: string;
    idempotencyKey: string;
    timeoutMs?: number;
}) {
    const finalP = onceMessage(socket, (o) => o.type === "res" && o.id === params.reqId && o.payload?.status !== "accepted", params.timeoutMs);
...
describe("gateway server agent", () => {
    ...
    test("agent dedupe survives reconnect", { timeout: 20000 }, async () => {
        await withGatewayServer(async ({ port }) => {
            const dial = async () => {
    ...
    test("agent events stream to webchat clients when run context is registered", async () => {
        ...
        const finalChatP = onceMessage(webchatWs, (o) => {
