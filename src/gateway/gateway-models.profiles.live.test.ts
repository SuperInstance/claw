...
import { type AuthProfileStore, ensureAuthProfileStore, resolveAuthProfileOrder, saveAuthProfileStore, } from "../agents/auth-profiles.js";
import { collectAnthropicApiKeys, isAnthropicBillingError, isAnthropicRateLimitError, } from "../agents/live-auth-keys.js";
...
import { hasExpectedSingleNonce, hasExpectedToolNonce, shouldRetryExecReadProbe, shouldRetryToolReadProbe, } from "./live-tool-probe-utils.js";
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(30000, toInt(process.env.OPENCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90000));
...
function resolveGatewayLiveSuiteTimeoutMs(maxModels: number): number {
    ...
    return Math.max(GATEWAY_LIVE_DEFAULT_TIMEOUT_MS, Math.min(GATEWAY_LIVE_MAX_TIMEOUT_MS, estimated));
}
...
function capByProviderSpread<T>(items: T[], maxItems: number, providerOf: (item: T) => string): T[] {
...
function formatFailurePreview(failures: Array<{
    model: string;
    error: string;
...
function assertNoReasoningTags(params: {
    ...
    if (THINKING_TAG_RE.test(params.text) || FINAL_TAG_RE.test(params.text)) {
        ...
        throw new Error(`[${params.label}] reasoning tag leak (${params.model} / ${params.phase}): ${snippet}`);
    }
}
...
function isProviderUnavailableErrorMessage(raw: string): boolean {
    const msg = raw.toLowerCase();
    return (msg.includes("no allowed providers are available") ||
...
async function runAnthropicRefusalProbe(params: {
    ...
    const probe = await withGatewayLiveProbeTimeout(params.client.request<AgentFinalPayload>("agent", {
        sessionKey: params.sessionKey,
        idempotencyKey: `idem-${runId}-refusal`,
        message: `Reply with the single word ok. Test token: ${magic}`,
        thinking: params.thinkingLevel,
        deliver: false,
    }, { expectFinal: true }), `${params.label}: refusal-probe`);
    if (probe?.status !== "ok") {
    ...
    const followup = await withGatewayLiveProbeTimeout(params.client.request<AgentFinalPayload>("agent", {
        sessionKey: params.sessionKey,
        idempotencyKey: `idem-${followupId}-refusal-followup`,
        message: "Now reply with exactly: still ok.",
        thinking: params.thinkingLevel,
        deliver: false,
    }, { expectFinal: true }), `${params.label}: refusal-followup`);
...
async function getFreeGatewayPort(): Promise<number> {
    ...
    for (let attempt = 0; attempt < 25; attempt += 1) {
        ...
        const ok = (await Promise.all(candidates.map((candidate) => isPortFree(candidate)))).every(Boolean);
        if (ok) {
            return port;
        }
    }
...
async function connectClient(params: {
    ...
    return await new Promise<GatewayClient>((resolve, reject) => {
        ...
        const timer = setTimeout(() => stop(new Error("gateway connect timeout")), 10000);
        timer.unref();
        client.start();
    });
}
...
function buildLiveGatewayConfig(params: {
    ...
    return {
        ...
        models: Object.keys(providers).length > 0
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
    ...
    try {
        const port = await withGatewayLiveProbeTimeout(getFreeGatewayPort(), `${params.label}: gateway-port`);
        server = await withGatewayLiveProbeTimeout(startGatewayServer(port, {
            bind: "loopback",
            auth: { mode: "token", token },
            controlUiEnabled: false,
        }), `${params.label}: gateway-start`);
        client = await withGatewayLiveProbeTimeout(connectClient({
    ...
    try {
        logProgress(`[${params.label}] running ${params.candidates.length} models (thinking=${params.thinkingLevel})`);
        ...
        for (const [index, model] of params.candidates.entries()) {
            ...
            for (let attempt = 0; attempt < attemptMax; attempt += 1) {
                ...
                try {
                    ...
                    await withGatewayLiveProbeTimeout(client.request("sessions.reset", {
                        key: sessionKey,
                    }), `${progressLabel}: sessions-reset`);
                    await withGatewayLiveProbeTimeout(client.request("sessions.patch", {
                        key: sessionKey,
                        model: modelKey,
                    }), `${progressLabel}: sessions-patch`);
                    logProgress(`${progressLabel}: prompt`);
                    const runId = randomUUID();
                    const payload = await withGatewayLiveProbeTimeout(client.request<AgentFinalPayload>("agent", {
                        sessionKey,
                        ...
                        thinking: params.thinkingLevel,
                        deliver: false,
                    }, { expectFinal: true }), `${progressLabel}: prompt`);
                    ...
                    if (!text) {
                        ...
                        const retry = await withGatewayLiveProbeTimeout(client.request<AgentFinalPayload>("agent", {
                            sessionKey,
                            idempotencyKey: `idem-${randomUUID()}-retry`,
                            message: "Explain in 2-3 sentences how the JavaScript event loop handles microtasks vs macrotasks. Must mention both words: microtask and macrotask.",
                            thinking: params.thinkingLevel,
                            deliver: false,
                        }, { expectFinal: true }), `${progressLabel}: prompt-retry`);
                        if (retry?.status !== "ok") {
                    ...
                    let toolText = "";
                    for (let toolReadAttempt = 0; toolReadAttempt < maxToolReadAttempts; toolReadAttempt += 1) {
                        const strictReply = toolReadAttempt > 0;
                        const toolProbe = await withGatewayLiveProbeTimeout(client.request<AgentFinalPayload>("agent", {
                            sessionKey,
                            ...
                            thinking: params.thinkingLevel,
                            deliver: false,
                        }, { expectFinal: true }), `${progressLabel}: tool-read`);
                        if (toolProbe?.status !== "ok") {
                            if (toolReadAttempt + 1 < maxToolReadAttempts) {
                                logProgress(`${progressLabel}: tool-read retry (${toolReadAttempt + 2}/${maxToolReadAttempts}) status=${String(toolProbe?.status)}`);
                                continue;
                            }
                        ...
                        if (shouldRetryToolReadProbe({
                            ...
                            logProgress(`${progressLabel}: tool-read retry (${toolReadAttempt + 2}/${maxToolReadAttempts}) malformed tool output`);
                            continue;
                        }
                    ...
                    if (params.extraToolProbes) {
                        ...
                        let execReadText = "";
                        for (let execReadAttempt = 0; execReadAttempt < maxExecReadAttempts; execReadAttempt += 1) {
                            const strictReply = execReadAttempt > 0;
                            const execReadProbe = await withGatewayLiveProbeTimeout(client.request<AgentFinalPayload>("agent", {
                                sessionKey,
                                ...
                                thinking: params.thinkingLevel,
                                deliver: false,
                            }, { expectFinal: true }), `${progressLabel}: tool-exec`);
                            if (execReadProbe?.status !== "ok") {
                                if (execReadAttempt + 1 < maxExecReadAttempts) {
                                    logProgress(`${progressLabel}: tool-exec retry (${execReadAttempt + 2}/${maxExecReadAttempts}) status=${String(execReadProbe?.status)}`);
                                    continue;
                                }
                            ...
                            if (shouldRetryExecReadProbe({
                                ...
                                logProgress(`${progressLabel}: tool-exec retry (${execReadAttempt + 2}/${maxExecReadAttempts}) malformed tool output`);
                                continue;
                            }
                    ...
                    if (params.extraImageProbes && model.input?.includes("image")) {
                        ...
                        const imageProbe = await withGatewayLiveProbeTimeout(client.request<AgentFinalPayload>("agent", {
                            sessionKey,
                            ...
                            thinking: params.thinkingLevel,
                            deliver: false,
                        }, { expectFinal: true }), `${progressLabel}: image`);
                    ...
                    if ((model.provider === "openai" && model.api === "openai-responses") ||
                        ...
                        const first = await withGatewayLiveProbeTimeout(client.request<AgentFinalPayload>("agent", {
                            sessionKey,
                            idempotencyKey: `idem-${runId2}-1`,
                            message: `Call the tool named \`read\` (or \`Read\`) on "${toolProbePath}". Do not write any other text.`,
                            thinking: params.thinkingLevel,
                            deliver: false,
                        }, { expectFinal: true }), `${progressLabel}: tool-only-regression-first`);
                        if (first?.status !== "ok") {
                        ...
                        const second = await withGatewayLiveProbeTimeout(client.request<AgentFinalPayload>("agent", {
                            sessionKey,
                            ...
                            thinking: params.thinkingLevel,
                            deliver: false,
                        }, { expectFinal: true }), `${progressLabel}: tool-only-regression-second`);
                        if (second?.status !== "ok") {
        ...
        if (failures.length > 0) {
            ...
            throw new Error(`gateway live model failures (${failures.length}, showing ${Math.min(failures.length, 20)}):\n${preview}`);
        }
...
describeLive("gateway live (dev agent, profile keys)", () => {
    it("runs meaningful prompts across models with available keys", async () => {
        ...
        const selectedCandidates = capByProviderSpread(candidates, maxModels > 0 ? maxModels : candidates.length, (model) => model.provider);
        logProgress(`[all-models] selection=${useExplicit ? "explicit" : "modern"}`);
        if (selectedCandidates.length < candidates.length) {
            logProgress(`[all-models] capped to ${selectedCandidates.length}/${candidates.length} via OPENCLAW_LIVE_GATEWAY_MAX_MODELS=${maxModels}`);
        }
    ...
    it("z.ai fallback handles anthropic tool history", async () => {
        ...
        let client: GatewayClient | undefined;
        try {
            const port = await withGatewayLiveProbeTimeout(getFreeGatewayPort(), "zai-fallback: gateway-port");
            server = await withGatewayLiveProbeTimeout(startGatewayServer(port, {
                bind: "loopback",
                auth: { mode: "token", token },
                controlUiEnabled: false,
        ...
        try {
            ...
            const toolProbe = await withGatewayLiveProbeTimeout(client.request<AgentFinalPayload>("agent", {
                ...
                message: `Call the tool named \`read\` (or \`Read\` if \`read\` is unavailable) with JSON arguments {"path":"${toolProbePath}"}. ` +
                    `Then reply with exactly: ${nonceA} ${nonceB}. No extra text.`,
                thinking: THINKING_LEVEL,
                deliver: false,
            }, { expectFinal: true }), "zai-fallback: tool-probe");
            if (toolProbe?.status !== "ok") {
            ...
            const followupId = randomUUID();
            const followup = await withGatewayLiveProbeTimeout(client.request<AgentFinalPayload>("agent", {
                sessionKey,
                idempotencyKey: `idem-${followupId}-followup`,
                message: `What are the values of nonceA and nonceB in "${toolProbePath}"? ` +
                    `Reply with exactly: ${nonceA} ${nonceB}.`,
                thinking: THINKING_LEVEL,
                deliver: false,
            }, { expectFinal: true }), "zai-fallback: followup");
            if (followup?.status !== "ok") {
