...
describe("resolveDeliveryTarget", () => {
    ...
    it("applies id-like target normalization before returning delivery targets", async () => {
        ...
        expect(maybeResolveIdLikeTarget).toHaveBeenCalledWith(expect.objectContaining({
            channel: "telegram",
            input: "123456789",
        }));
    });
    ...
    it("returns an error when channel selection is ambiguous", async () => {
        setMainSessionEntry(undefined);
        vi.mocked(resolveMessageChannelSelection).mockRejectedValueOnce(new Error("Channel is required when multiple channels are configured: telegram, slack"));
        const result = await resolveLastTarget(makeCfg({ bindings: [] }));
