...
import { getModelRefStatus, isCliProvider, normalizeModelSelection, resolveAllowedModelRef, resolveConfiguredModelRef, resolveHooksGmailModel, resolveThinkingDefault, } from "../../agents/model-selection.js";
...
import { resolveSessionTranscriptPath, setSessionRuntimeModel, updateSessionStore, } from "../../config/sessions.js";
...
import { dispatchCronDelivery, matchesMessagingToolDeliveryTarget, resolveCronDeliveryBestEffort, } from "./delivery-dispatch.js";
...
function extractCronAgentDefaultsOverride(agentConfigOverride?: ResolvedAgentConfig) {
    ...
    return {
        overrideModel,
        definedOverrides: Object.fromEntries(Object.entries(agentOverrideRest).filter(([, value]) => value !== undefined)) as Partial<AgentDefaultsConfig>,
    };
}
...
export async function runCronIsolatedAgentTurn(params: {
    ...
    const subagentModelRaw = normalizeModelSelection(agentConfigOverride?.subagents?.model) ??
    ...
    const withRunSession = (result: Omit<RunCronAgentTurnResult, "sessionId" | "sessionKey">): RunCronAgentTurnResult => ({
        ...result,
    ...
    try {
        ...
        const payloadFallbacks = params.job.payload.kind === "agentTurn" && Array.isArray(params.job.payload.fallbacks)
        ...
        if (!isAborted()) {
            ...
            const interimPayloadHasStructuredContent = Boolean(interimDeliveryPayload?.mediaUrl) ||
            ...
            const shouldRetryInterimAck = !interimRunResult.meta?.error &&
    ...
    const hasSuccessfulPayloadAfterLastError = !runLevelError &&
    ...
    const skipMessagingToolDelivery = deliveryContract === "shared" &&
        deliveryRequested &&
        finalRunResult.didSendViaMessagingTool === true &&
        (finalRunResult.messagingToolSentTargets ?? []).some((target) => matchesMessagingToolDeliveryTarget(target, {
