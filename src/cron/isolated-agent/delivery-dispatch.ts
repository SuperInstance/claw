...
import { deliverOutboundPayloads, type OutboundDeliveryResult, } from "../../infra/outbound/deliver.js";
...
export function matchesMessagingToolDeliveryTarget(target: {
    provider?: string;
    to?: string;
    accountId?: string;
}, delivery: {
...
export type SuccessfulDeliveryTarget = Extract<DeliveryTargetResolution, {
    ok: true;
}>;
...
function cloneDeliveryResults(results: readonly OutboundDeliveryResult[]): OutboundDeliveryResult[] {
...
function pruneCompletedDirectCronDeliveries(now: number) {
    const ttlMs = process.env.OPENCLAW_TEST_FAST === "1" ? 60000 : 24 * 60 * 60 * 1000;
...
function rememberCompletedDirectCronDelivery(idempotencyKey: string, results: readonly OutboundDeliveryResult[]) {
    const now = Date.now();
...
function getCompletedDirectCronDelivery(idempotencyKey: string): OutboundDeliveryResult[] | undefined {
    const now = Date.now();
    pruneCompletedDirectCronDeliveries(now);
...
function buildDirectCronDeliveryIdempotencyKey(params: {
    ...
    const threadId = params.delivery.threadId == null || params.delivery.threadId === ""
        ? ""
...
async function retryTransientDirectCronDelivery<T>(params: {
    ...
    for (;;) {
        ...
        try {
            ...
            logWarn(`[cron:${params.jobId}] transient direct announce delivery failure, retrying ${nextAttempt}/${maxAttempts} in ${Math.round(delayMs / 1000)}s: ${summarizeDirectCronDeliveryError(err)}`);
            retryIndex += 1;
...
export async function dispatchCronDelivery(params: DispatchCronDeliveryParams): Promise<DispatchCronDeliveryState> {
    ...
    const failDeliveryTarget = (error: string) => params.withRunSession({
        status: "error",
        error,
    ...
    const deliverViaDirect = async (delivery: SuccessfulDeliveryTarget, options?: {
    ...
    const finalizeTextDelivery = async (delivery: SuccessfulDeliveryTarget): Promise<RunCronAgentTurnResult | null> => {
        ...
        const completedDescendantReply = activeSubagentRuns === 0 && isLikelyInterimCronMessage(initialSynthesizedText)
