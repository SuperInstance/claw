...
export function resolveCronDeliveryPlan(job: CronJob): CronDeliveryPlan {
    ...
    const rawMode = hasDelivery ? (delivery as {
        mode?: unknown;
    }).mode : undefined;
    ...
    const deliveryChannel = normalizeChannel((delivery as {
        channel?: unknown;
    } | undefined)?.channel);
    const deliveryTo = normalizeTo((delivery as {
        to?: unknown;
    } | undefined)?.to);
    const channel = deliveryChannel ?? payloadChannel ?? "last";
    const to = deliveryTo ?? payloadTo;
    const deliveryAccountId = normalizeAccountId((delivery as {
        accountId?: unknown;
    } | undefined)?.accountId);
    if (hasDelivery) {
...
export function resolveFailureDestination(job: CronJob, globalConfig?: CronFailureDestinationConfig): CronFailureDeliveryPlan | null {
    const delivery = job.delivery;
...
export async function sendFailureNotificationAnnounce(deps: CliDeps, cfg: OpenClawConfig, agentId: string, jobId: string, target: {
    channel?: string;
    to?: string;
    accountId?: string;
